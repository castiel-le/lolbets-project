 ...
 * Determines whether to show the contents of the selected tab or not
 */
...
/**
 * MUI tabs container styled to have our colors
 */
...
/**
 * MUI single tab styled to have our colors
 */
...
/**
 * Holds tabs for matchups and custom bets
 * matchups state is shared between the matchups tab and custom bets tab to avoid multiple fetches
 * @param user the user currently stored inside APP1. Used to determine if they are logged in
 * @param updateUser callback used when the application should update the user state
 */
export default function BetsPage({user, updateUser}) {
    ...
    /**
     * When component mounts, fetch the upcoming matches
     */
    useEffect(() => {
    ...
    /**
     * If the state of fetchNextDate changes, determine what you should do next
     */
    useEffect(() => {
        if (fetchNextDate) {
            fetchUpcomingMatches(1);
    ...
    /**
     * Sets the current tab to be the one selected
     * @param {*} event 
     * @param {*} tabNumber the tab that the user selected
     */
